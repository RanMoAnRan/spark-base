...
/**
  ...
  * 窗口统计
  */
object StreamingOrderCntWindow {

  // Streaming检查点目录
  val CKPT_DIR: String = "datas/streaming/state/order-amt-100001111111111112"
  ...
  /**
    * 将Streaming流式应用从数据源数据，数据处理分析及结构数据输出封装到函数中
    *
    */
  def processStreaming(ssc: StreamingContext): Unit = {
    ...
    val orderDStream: DStream[(String, Int)] = windowDStream.transform(rdd => {
