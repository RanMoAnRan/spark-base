...
/**
  ...
  * 循环停止启动streaming程序  每当次日凌晨就会停止当前应用，重启应用
  */
object StreamingOrderStateDaily {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 死循环，每当次日凌晨就会停止当前应用，重启应用
    while (true) {
      // Streaming检查点目录
      val CKPT_DIR: String = s"/datas/streaming/state/order-amt2-${new Date().getTime}"
      ...
      // TODO: 计算Streaming应用启动时距离次日凌晨时长
      val restInterval: Long = {
        // 获取当前日期时间
        val nowDate: Date = new Date()
        // 通过当前日期获取次日日期
        val nextDate: Date = new Date(nowDate.getYear, nowDate.getMonth, nowDate.getDate + 1)
        // 计算时长
        nextDate.getTime - nowDate.getTime
      }
      println(s"restInterval = $restInterval")

      // 等待运行restInterval
      context.awaitTerminationOrTimeout(restInterval)

      // 停止应用，此处SparkContext启动时很慢
