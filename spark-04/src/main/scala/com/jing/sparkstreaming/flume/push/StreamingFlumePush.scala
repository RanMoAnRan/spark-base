package com.jing.sparkstreaming.flume.push
...
/**
  * Streaming实时统计每批次数据中词频统计WordCount, 集成Flume，采用Push方式读取数据，将每批次结果打印到控制台。
  */
object StreamingFlumePush {
	...
	def main(args: Array[String]): Unit = {
		 ...
		 */
		val flumeDStream: DStream[SparkFlumeEvent] = FlumeUtils.createStream(
			//因为是push方式接收数据，所以此ip为本机windosip
			ssc, "10.4.16.42", 9999, storageLevel = StorageLevel.MEMORY_AND_DISK
		)
